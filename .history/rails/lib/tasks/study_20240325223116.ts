...
// ----------------------cut---
function simulateAsyncOperation(isSuccessful: boolean) {
  return new Promise((resolve, reject) => {
      setTimeout(() => {
          if (isSuccessful) {
              ...
              // ここを完成させてください
              // reject()の場合は、 catch(err) でしか取得されない!!
...
// const result = simulateAsyncOperation(true);
// console.log('simulateAsyncOperation:',result);   NG Promise: {}が返ってくる
...
async function handleOperation() {
  try {
      // const rlt = await simulateAsyncOperation(false)
      const rlt = await simulateAsyncOperation(true);
      console.log('rlt:::', rlt);
  } catch(err) {
      console.log('err:::', err);
  }
